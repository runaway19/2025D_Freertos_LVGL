...
u16 AD5933_Tempter(void)
{
    unsigned int Tm;   //温度
		...
		while(1)
		{
				ReadTemp = Rece_Byte(0x8F); //读取状态寄存器检查T
				if (ReadTemp & 0x01)
						break;
		}
...
float Correct_Rmeasure(float *Rreal,float *Rmeasure,u8 len,float Rcurrent)
{
	float Rcorrect = 0;
	if(Rcurrent >= Rmeasure[0] && Rcurrent <= Rmeasure[len - 1])
	{
		for(int i=0;i<(len - 1);i++)
		{
			if(Rcurrent >= Rmeasure[i] && Rcurrent <= Rmeasure[i+1])
			{
				Rcorrect = (Rreal[i+1]-Rreal[i])/(Rmeasure[i+1]-Rmeasure[i])
				* (Rcurrent - Rmeasure[i]) + Rreal[i];
			}
		}
	}
	return Rcorrect;
...
//float Get_uC(float src[], u16 num)
//{
//	u16 i, cnt = 0;
//    float sum;
...
float Scale_imp (u8 *SValue, u8 *IValue, u8 *NValue, u8 *CValue)
{
    ...
    while(1)
    {
        ...
        AD5933_Dat_Im[j] = imageArr[2];			//将虚部数据存储到数组AD5933_Dat_Im
				rads[j] = atan2((float)imageArr[2], (float)realArr[2]);
...
注  释：无
*/											  
float Rreal[LEN] =    {75.03 ,98.822 ,147.44  , 329.37  , 1000.3 , 3265.0  ,9943.3 , 11072.0};
float Rmeasure[LEN] = {76.95 ,100.1  ,147.45  , 327.0   , 979.92 , 3172.9  ,9896.0 , 10580.0};										  
float DA5933_Get_RsRealImage(float *R_Impedance, float *R_Rad, float *realData, float *imageData)
{
	...
	*R_Impedance = Correct_Rmeasure(Rreal,Rmeasure,LEN,RsWord * AD5933_Correction);
	//*R_Rad = RsWord * AD5933_Correction;
	//*R_Impedance = RsWord * AD5933_Correction_100R;	//阻抗数据乘以一个校准因子，得到电阻值(fudu)
